...
// export type OP_TYPE = 'update' | 'detail';

export default function XXUpload(props: any) {
  const { opType, type, accept, form, fileList, onChange, detail } = props;
  const cls = classNames('xx-upload', opType);

  if (opType === 'detail') {
    const list = type === 'picture' ? detail?.picture : detail?.video;
    return (
      <div className={cls}>
        {
          (list || []).map((item: any) => (
  ...
  return (
    <div className={cls}>
      <Upload
        // listType={opType === 'update' ? 'text' : 'picture-card'}
        listType='text'
        accept={accept}
        fileList={fileList}
        onChange={({ fileList }) => {
